...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.1.0) Spawn events from a specified spawn map to a desired location.
...
Galv.SPAWN.event = function(eventId,type,data,overlap,save) {
...
Spriteset_Map.prototype.unspawnEvent = function(eId) {
	for (let i = 0; i < this._characterSprites.length; i++) {
		...
		if (event.isSpawnEvent && eId == event._eventId) {
			//for (let s = 0; s < this._characterSprites[i]._animationSprites.length; s++) {
			//	this._tilemap.removeChild(this._characterSprites[i]._animationSprites[s]);
			//};
			// Stop animation needs fixing!
